...
''' CODE GOLF COMPILER - I know this code is not well-formated and easy to read but the main challenge was to write a compiler in under 500 lines '''
memoryInd, bufferId, instructions, initialized, declared, iterators, indexes, uiterators, arrays, usages, memory, instructionBuffer, rtol = 0, 0, [], [], [], [], [], [], {}, {}, {}, {}, ["a", "b", "c", "d", "e", "f"]
...
def add_commands(commands=None, setBuffer=-1):
    ...
    if bufferId != 0:
        if bufferId not in instructionBuffer: instructionBuffer[bufferId] = []
        instructionBuffer[bufferId] += commands if commands is not None else []
    else: instructions += commands if commands is not None else []
...
def set_register(reg, val, bid):
    ...
    for ind, dig in enumerate(binn):
        if dig == '1': add_commands([f"INC {rtol[reg]}"], bid)
        if ind < len(binn) - 1: add_commands([f"SHL {rtol[reg]}"], bid)
...
def assign(a, b, bid):
    if a in iterators and a in declared: raise AlreadyDeclared(a)
    if a in iterators and a in memory: raise IteratorModification(a)
    elif a in iterators and a not in memory: raise NotDeclared(a)
    if isinstance(b, str): mem_to_reg(1, memory[b], bid)
    elif isinstance(b, int): set_register(1, b, bid)
    elif isinstance(b, tuple) and b[0] == 'tab': mem_to_reg(1, memory[b[1]] + b[2], bid)
    elif isinstance(b, tuple) and b[0] == 'tabi': obtain_tab_value(b, 1, bid)

    if isinstance(a, str): reg_to_mem(1, memory[a], bid)
    elif isinstance(a, tuple) and a[0] == 'tab': reg_to_mem(1, memory[a[1]] + a[2], bid)
...
def evaluate_io(a, reg, bid, isRead):
    global memoryInd
    if isinstance(a, str):
        if a in iterators and a in memory and isRead: raise IteratorModification(a)
        ...
        memoryInd += 1
    elif isinstance(a, tuple) and a[0] == 'tab': set_register(0, memory[a[1]] + a[2], bid)
    elif isinstance(a, tuple) and a[0] == 'tabi': obtain_tab_address(a, reg, bid)
...
def evaluate_operation(a, b, c, bid, performOperation=True, loadB=True, loadA=True):
    if isinstance(a, int) and isinstance(b, int) and performOperation: set_register(1, c, bid)
    else:
        for ev in to_load(a, b, loadA, loadB):
            if isinstance(ev[0], tuple) and ev[0][0] == 'tab': mem_to_reg(ev[1], memory[ev[0][1]] + ev[0][2], bid)
            elif isinstance(ev[0], tuple) and ev[0][0] == 'tabi': obtain_tab_value(ev[0], ev[1], bid)
            elif isinstance(ev[0], int): set_register(ev[1], ev[0], bid)
            else: mem_to_reg(ev[1], memory[ev[0]], bid)
        return True

def neq(a, b, bid):
    if evaluate_operation(a, b, int(a != b) if isinstance(a, int) and isinstance(b, int) else -1, bid): add_commands(["RESET d", "ADD d c", "SUB d b", "SUB b c", "ADD b d"], bid)
...
def ge(a, b, bid):
    if evaluate_operation(a, b, int(a >= b) if isinstance(a, int) and isinstance(b, int) else -1, bid): add_commands(["INC b", "SUB b c"], bid)

def le(a, b, bid):
    if evaluate_operation(a, b, int(a <= b) if isinstance(a, int) and isinstance(b, int) else -1, bid): add_commands(["INC c", "SUB c b", "RESET b", "ADD b c"], bid)

def gt(a, b, bid):
    if evaluate_operation(a, b, int(a > b) if isinstance(a, int) and isinstance(b, int) else -1, bid): add_commands(["SUB b c"], bid)

def lt(a, b, bid):
    if evaluate_operation(a, b, int(a < b) if isinstance(a, int) and isinstance(b, int) else -1, bid): add_commands(["SUB c b", "RESET b", "ADD b c"], bid)

def _if(b):
    add_commands([f"JZERO b {len(instructionBuffer[b]) + 1}"] + instructionBuffer[b], b - 1)

def _ifelse(b1, b2):
    add_commands([f"JZERO b {len(instructionBuffer[b1]) + 2}"] + instructionBuffer[b1] + [f"JUMP {len(instructionBuffer[b2]) + 1}"] + instructionBuffer[b2], b1 - 1)
...
def add(a, b, bid):
    if b != 0 and evaluate_operation(a, b, a + b if isinstance(a, int) and isinstance(b, int) else -1, bid, loadA=(a != 0 and a != 1), loadB=(b != 0 and b != 1)):
        if b == 1: add_commands(["INC b"], bid)
        elif a == 0 or a == 1: add_commands(["RESET b", "ADD b c"] if a == 0 else ["RESET b", "ADD b c", "INC b"], bid)
        elif b != 0: add_commands(["ADD b c"], bid)
...
def sub(a, b, bid):
    if b != 0 and evaluate_operation(a, b, a - b if isinstance(a, int) and isinstance(b, int) and a - b >= 0 else 0, bid, loadA=(a != 0 and a != 1), loadB=(b != 0 and b != 1)):
        if b == 1: add_commands(["DEC b"], bid)
        elif a == 0 or a == 1: add_commands(["RESET b", "ADD b c"] if a == 0 else ["RESET b", "ADD b c", "DEC b"], bid)
        elif b != 0: add_commands(["SUB b c"], bid)

def mul(a, b, bid):
    if a == 0 or b == 0: return set_register(1, 0, bid)
    if evaluate_operation(a, b, a * b if isinstance(a, int) and isinstance(b, int) else -1, bid, loadA=(a != 1 and not (isinstance(a, int) and a & (a - 1) == 0)),
        ...
        if a == 1: add_commands(["RESET b", "ADD b c"], bid)
        elif b == 1: return
        ...
        else: add_commands(["RESET e", "ADD e b", "SUB b c", "JZERO b 10", "RESET b", "JODD c 2", "JUMP 2","ADD b e", "SHL e", "SHR c",
...
def div(a, b, bid, modulo=False):
    ...
    if evaluate_operation(a, b, c, bid, loadB=((b != 1 and not (isinstance(b, int) and b & (b - 1) == 0)) or modulo) and (b != 1 or modulo)):
        ...
        else: add_commands(['JZERO c 27', 'RESET d', 'ADD d c', 'RESET f', 'ADD f d', 'SUB f b', 'JZERO f 2', 'JUMP 3', 'SHL d', 'JUMP -6', 'RESET f',
...
for key, value in dyn_tokens.items():
    if value is not None: vars()['t_' + key] = value if value != '' else key
...
def p_declarations_array(p):
    ...
    if p[ind + 2] > p[ind + 4]: raise InvalidArrayRange(p.lineno(ind), p[ind + 2], p[ind + 4])
    p[0] = p[ind] if p[ind] not in declared else False
    if not p[0]: raise AlreadyDeclared(p[ind], p.lineno(ind))
...
def init_val(val, lineno):
    if val in iterators and val in declared: raise AlreadyDeclared(val, lineno)
    if isinstance(val, tuple):
        if val[1] not in initialized:
            initialized.append(val[1])
            declare_variable(val)
        if val[0] == 'tab' and val not in initialized: initialized.append(val)
...
def check_variable(var, lineno):
    if isinstance(var, tuple):
        if var[1] not in initialized: raise NotInitialized(var[1], lineno)
        if var[0] == 'tabi': return check_variable(var[2], lineno)
    elif not isinstance(var, int) and not isinstance(var, tuple) and var not in declared and var not in iterators: raise NotDeclared(var, lineno)
    elif not isinstance(var, int) and var not in initialized and var not in iterators: raise NotInitialized(var, lineno)

def p_command_assign(p):
    ''' command	: identifier ASSIGN expression SEMICOLON '''
    if isinstance(p[3], str) or (isinstance(p[3], tuple) and (p[3][0] == 'tab' or p[3][0] == 'tabi')): check_variable(p[3], p.lineno(2))
    if p[1] not in iterators: init_val(p[1], p.lineno(1))
    p[0] = ("assign", p[1], p[3])
...
def p_iterator(p):
    ''' iterator	: PID '''
    p[0] = p[1]
    if p[1] in memory: raise AlreadyDeclared(p[1], p.lineno(1))
...
def p_command_for(p):
    ...
    if p[2] in declared: raise AlreadyDeclared(p[2], p.lineno(2))
...
def p_command_io(p):
    ...
    if p[1] == 'READ' and p[2] not in iterators: init_val(p[2], p.lineno(2))
    elif p[1] == 'WRITE': check_variable(p[2], p.lineno(1))
...
def p_identifier_pid(p):
    '''identifier	: PID '''
    if p[1] not in declared and p[1] not in iterators: raise NotDeclared(p[1], p.lineno(1))
    if p[1] in arrays: raise NotAVariable(p.lineno(1), p[1])
    p[0] = (p[1])

def p_identifier_tab_id(p):
    '''identifier	: PID LBRACKET PID RBRACKET
                    | PID LBRACKET NUM RBRACKET '''
    if p[1] not in declared: raise NotDeclared(p[1], p.lineno(1))
    if p[1] not in arrays: raise NotAnArray(p.lineno(1), p[1])
    if isinstance(p[3], int) and (p[3] < arrays[p[1]][0] or p[3] > arrays[p[1]][1]): raise InvalidArrayElement(p.lineno(1), p[3], p[1])
    p[0] = ("tab", p[1], (p[3])) if isinstance(p[3], int) else ("tabi", p[1], (p[3]))

def p_error(p):
    if hasattr(p, 'type'): raise InvalidCharacter(str(p.lineno), str(p.value))
...
def evaluate(ex, bid=0, justassign=False):
    ...
    if isinstance(ex, list):
        ...
        if ex[0] == 'read' or ex[0] == 'write':
            ...
            if ex[1] in initialized: raise ReusedIterator(ex[1])
            ...
            evaluate(ex[1], bid)
            evaluate(ex[2], bid + 1)
            if ex[0] == 'ifelse': evaluate(ex[3], bid + 2)
            _if(bid + 1) if ex[0] == 'if' else _ifelse(bid + 1, bid + 2)
            clear_ins_buffer(bid + 1)
            if ex[0] == 'ifelse': clear_ins_buffer(bid + 2)
        elif ex[0] == 'assign':
            ...
            if ex[0] == 'tab': return (ex[0], ex[1], ex[2] - arrays[ex[1]][0])
            else: indexes.append(memory[evaluate(ex[2], bid)])
    else:
        if isinstance(ex, str) and ex not in declared and ex not in iterators and not justassign: raise NotDeclared(str(ex))
        if isinstance(ex, str) and ex in iterators and ex not in uiterators and not justassign: raise NotDeclared(ex)
        if isinstance(ex, str) and ex not in initialized and not justassign: raise NotInitialized(ex)
    return ex
